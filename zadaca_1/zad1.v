...
-- rewrite IHx.
--- reflexivity.
---  Search (S _ = S _). apply Nat.succ_inj in H.
 exact H.
Qed.
...
Proof.
intros [H [H0]]. simpl. rewrite H, H0. 
assert (P_NotL : forall (n : nat), NotL (repeat O n) = repeat I n).
{intros. induction n0.
 - reflexivity.
...
- simpl. rewrite IHn0. rewrite P_rev_1. reflexivity.
 ...
 assert(S_AddL: forall (n : nat), I :: repeat I n = repeat I ( S n)).
{ intros. simpl. reflexivity.
}
assert (HAddLr:  forall (n : nat), AddLr (repeat I n) (repeat I n) I = repeat I n).
{ 
intros. induction n0.
- simpl. reflexivity.
- simpl. rewrite IHn0. reflexivity.
} 
assert(AdddL: AddL (repeat I n) (repeat I n) = repeat I (pred n) ++ [O]).
{intros. induction n ; simpl.
- unfold AddL. simpl.  contradiction.
- unfold AddL. rewrite S_AddL. rewrite P_rev_2. simpl. rewrite HAddLr. rewrite P_rev_2. reflexivity.
} 
assert(NotL2: forall (n : nat), NotL (repeat I n ++ [O]) = repeat O n ++ [I]).
...
induction n ; simpl.
- contradiction.
- rewrite S_AddL, AdddL. simpl. rewrite NotL2. reflexivity.
Qed.
